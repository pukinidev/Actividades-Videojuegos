...
def system_enemy_hunter_state(world: esper.World, player_entity: int, hunter_info:dict):
    ...
    for entity, (c_st, c_a, c_t, c_v) in components:
...
def _do_enemy_hunter_return(c_st: CEnemyHunterState, c_a: CAnimation, c_t: CTransform, c_v: CVelocity, hunter_info: dict):
    set_animation(c_a, "MOVE")
    c_v.vel = (c_st.start_pos - c_t.pos).normalize() * hunter_info["velocity_return"]
    dist_to_origin = c_st.start_pos.distance_to(c_st.start_pos)
    if dist_to_origin <= 2:
